...
History: PJN / 10-11-2004 1. Fix for CAADate::Get so that it works correctly for propalactive calendar dates
         ...
         PJN / 26-01-2006 1. After a bug report from Ing. Taras Kapuszczak that a round trip of the date 25 January 100 as 
                          ...
                          2. Optimized CAADateructor code
         ...
         PJN / 27-01-2007 1. The static version of the Set method has been renamed to DateToJD to avoid any confusion with
                          ...
                          AfterPapalReform.
                          3. Reinstated the bGregorianCalendar parameter for the CAADateructors and Set methods.
...
CAADate::CAADate(long Year, long Month, double Day, bool bGregorianCalendar) : m_dblJulian{0},
                                                                                        m_bGregorianCalendar{false}
{
  Set(Year, Month, Day, 0, 0, 0, bGregorianCalendar);
}

CAADate::CAADate(long Year, long Month, double Day, double Hour, double Minute, double Second, bool bGregorianCalendar) : m_dblJulian{0},
                                                                                                                                   m_bGregorianCalendar{false}
{
  Set(Year, Month, Day, Hour, Minute, Second, bGregorianCalendar);
}

CAADate::CAADate(double JD, bool bGregorianCalendar) : m_dblJulian{0},
                                                                m_bGregorianCalendar{false}
{
...
void CAADate::Get(long% Year, long% Month, long% Day, long% Hour, long% Minute, double% Second)
{
  ...
  //if (Z >= 2299161)       //The Meeus implementation automatically assumes the Gregorian Calendar 
                            ...
                            //Julian calendar. This allows you toruct Julian dates after the Papal
                            //reform in 1582. This is useful if you want toruct dates in countries
...
CAADate::DOW CAADate::DayOfWeek()
{
  long t{static_cast<long>(m_dblJulian + 1.5)};
  if (t >= 0)
...
long CAADate::DaysInMonth(long Month, bool bLeap)
{
  ...
  static std::array<int, 12> g_NonLeapMonths{31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
  static std::array<int, 12> g_LeapMonths{31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
