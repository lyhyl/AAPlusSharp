...
CAACalendarDate CAAMoslemCalendar::MoslemToJulian(long Year, long Month, long Day)
{
  ...
  long N{Day + CAADate::INT(29.5001*(Month - 1.0) + 0.99)};
  long Q{CAADate::INT(Year/30.0)};
  long R{Year % 30};
  long A{CAADate::INT(((11.0*R) + 3)/30.0)};
  long W{(404*Q) + (354*R) + 208 + A};
  long Q1{CAADate::INT(W/1461.0)};
  long Q2{W % 1461};
  long G{621 + (4*CAADate::INT((7.0*Q) + Q1))};
...
CAACalendarDate CAAMoslemCalendar::JulianToMoslem(long Year, long Month, long Day)
{
  ...
  long W{(Year % 4) ? 2 : 1};
  long N{CAADate::INT((275.0*Month)/9.0) - (W*CAADate::INT((Month + 9.0)/12.0)) + Day - 30};
  long A{Year - 623};
  ...
  long S{CAADate::INT((JJ - 1.0)/29.5)};
