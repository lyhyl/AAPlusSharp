...
History: PJN / 18-06-2022 1. Updated all the code in AADiameters.cpp to use C++ uniform initialization 
         ...
         PJN / 11-07-2023 1. Updated CAADiameters::GeocentricMoonSemidiameter method to use the more 
                         ...
                         ant used in this method from 0.272481 to the more modern value of k = 
...
double CAADiameters::TopocentricMoonSemidiameter(double DistanceDelta, double Delta, double H, double Latitude, double Height)
{
  ...
  double A{cosDelta*sin(H)};
  double sinPi{sin(pi)};
  double B{(cosDelta*cos(H)) - (CAAGlobe::RhoCosThetaPrime(Latitude, Height)*sinPi)};
  double C{sin(Delta) - (CAAGlobe::RhoSinThetaPrime(Latitude, Height)*sinPi)};
