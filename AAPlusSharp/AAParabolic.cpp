...
History: PJN / 31-01-2005 1. Fixed a bug in CAAParabolic::Calculate where the JD value was being used incorrectly
         ...
         PJN / 16-03-2008 1. Fixed a bug in CAAParabolic::Calculate(double JD, 
                          CAAParabolicObjectElements& elements) in the calculation of the 
...
double CAAParabolic::CalculateBarkers(double W, double epsilon)
{
  double S{W/3};
  bool bRecalc{true};
  while (bRecalc)
...
CAAParabolicObjectDetails CAAParabolic::Calculate(double JD, CAAParabolicObjectElements% elements, bool bHighPrecision, double epsilon)
{
  ...
  for (int j{0}; j<2; j++)
  {
    ...
    double s{CalculateBarkers(W, epsilon)};
    double v{2*atan(s)};
