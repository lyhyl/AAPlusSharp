...
CAAGalileanMoonsDetails CAAGalileanMoons::CalculateHelper(double JD, double sunlongrad, double betarad, double R, bool bHighPrecision)
{
  ...
  double twoG{2*G};
  double threeG{3*G};
  double twol3rad{2*l3rad};
  double twopsi{2*psi};
  double l1radminusl3rad{l1rad - l3rad};
  double l1radminuspi3{l1rad - pi3};
  double l1radminusl2rad{l1rad - l2rad};
  double l2radminusl3rad{l2rad - l3rad};
  double twol2rad{2*l2rad};
  double threel3rad{3*l3rad};
  double twol4rad{2*l4rad};
  double sevenl4rad{7*l4rad};
  double Sigma1{0.47259*sin(2*l1radminusl2rad) +
                      -0.03478*sin(pi3 - pi4) +
  ...
  double Sigma2{1.06476*sin(2*l2radminusl3rad) +
  ...
  double threeL3{3*L3};
  double four0threeSigma3rad{4.03*Sigma3rad};
...
void CAAGalileanMoons::Rotations(double X, double Y, double Z, double I, double psi, double i, double omega, double lambda0, double beta0, double% A6, double% B6, double% C6)
{
  ...
  double cosI{cos(I)};
  double sinI{sin(I)};
  double B1{(Y*cosI) - (Z*sinI)};
  double C1{(Y*sinI) + (Z*cosI)};

  //Rotation towards the ascending node of the orbit of Jupiter
  double cosphi{cos(phi)};
  double sinphi{sin(phi)};
  double A2{(A1*cosphi) - (B1*sinphi)};
  double B2{(A1*sinphi) + (B1*cosphi)};
  double C2{C1};
  ...
  double cosi{cos(i)};
  double sini{sin(i)};
  double B3{(B2*cosi) - (C2*sini)};
  double C3{(B2*sini) + (C2*cosi)};

  //Rotation towards the vernal equinox
  double cosomega{cos(omega)};
  double sinomega{sin(omega)};
  double A4{(A3*cosomega) - (B3*sinomega)};
  double B4{(A3*sinomega) + (B3*cosomega)};
  double C4{C3};

  double coslambda0(cos(lambda0));
  double sinlambda0(sin(lambda0));
  double A5{(A4*sinlambda0) - (B4*coslambda0)};
  double B5{(A4*coslambda0) + (B4*sinlambda0)};
  double C5{C4};
