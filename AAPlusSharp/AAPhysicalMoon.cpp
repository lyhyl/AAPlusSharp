...
void CAAPhysicalMoon::CalculateOpticalLibration(double JD, double Lambda, double Beta, double% ldash, double% bdash, double% ldash2, double% bdash2, double% epsilon, double% omega, double% DeltaU, double% sigma, double% I, double% rho)
{
  ...
  double cosBetarad{cos(Betarad)};
  double sinBetarad{sin(Betarad)};
  ...
  double W{Lambdarad - DeltaU - omega};
  double sinW{sin(W)};
  double A{atan2((sinW*cosBetarad*cosI) - (sinBetarad*sinI), cos(W)*cosBetarad)};
  double cosA{cos(A)};
  double sinA{sin(A)};
...
CAAPhysicalMoonDetails CAAPhysicalMoon::CalculateHelper(double JD, double% Lambda, double% Beta, double% epsilon, CAA2DCoordinate% Equatorial)
{
  ...
  double sinI_rho{sin(I_rho)};
  double X{sinI_rho*sin(V)};
  double Y{sinI_rho*cos(V)*cos(epsilonrad) - cos(I_rho)*sin(epsilonrad)};
  double w{atan2(X, Y)};
...
CAAPhysicalMoonDetails CAAPhysicalMoon::CalculateTopocentric(double JD, double Longitude, double Latitude)
{
  ...
  double Q{atan2(cosLatitude*sin(H), (cosDelta*sinLatitude) - (sinDelta*cosLatitude*cosH))};
  double Z{acos((sinDelta*sinLatitude) + (cosDelta*cosLatitude*cosH))};
