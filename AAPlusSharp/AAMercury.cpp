...
std::array<VSOP87Coefficient, 38> g_L0MercuryCoefficients
{ {
  { 440250710, 0,          0              },
...
std::array<VSOP87Coefficient, 16> g_L1MercuryCoefficients
{ {
  { 2608814706223.0, 0,         0             },
...
std::array<VSOP87Coefficient, 10> g_L2MercuryCoefficients
{ {
  { 53050, 0,       0           },
...
std::array<VSOP87Coefficient, 8> g_L3MercuryCoefficients
{ {
  { 188, 0.035, 52175.806 },
  { 142, 3.125, 26087.903 },
  { 97,  3.00,  78263.71  },
...
std::array<VSOP87Coefficient, 6> g_L4MercuryCoefficients
{ {
  { 114, 3.1416, 0         },
  { 3,   2.03,   26087.90  },
  { 2,   1.42,   78263.71  },
...
std::array<VSOP87Coefficient, 1> g_L5MercuryCoefficients
{ {
  { 1, 3.14, 0 }
} };

std::array<VSOP87Coefficient, 14> g_B0MercuryCoefficients
{ {
...
std::array<VSOP87Coefficient, 11> g_B1MercuryCoefficients
{ {
...
std::array<VSOP87Coefficient, 9> g_B2MercuryCoefficients
{ {
...
std::array<VSOP87Coefficient, 7> g_B3MercuryCoefficients
{ {
  { 235, 0.354, 26087.903 },
  { 161, 0,     0         },
  { 19,  4.36,  52175.81  },
...
std::array<VSOP87Coefficient, 2> g_B4MercuryCoefficients
{ {
  { 4, 1.75, 26087.90 },
  { 1, 3.14, 0        }
} };

std::array<VSOP87Coefficient, 13> g_R0MercuryCoefficients
{ {
  { 39528272, 0,         0             },
...
std::array<VSOP87Coefficient, 8> g_R1MercuryCoefficients
{ {
...
std::array<VSOP87Coefficient, 7> g_R2MercuryCoefficients
{ {
...
std::array<VSOP87Coefficient, 5> g_R3MercuryCoefficients
{ {
  { 33, 1.68, 26087.90  },
  { 24, 4.63, 52175.81  },
  { 12, 1.39, 78263.71  },
...
double CAAMercury::RadiusVector(double JD, bool bHighPrecision)
{
  ...
  //Calculate R0
  double R0{0};
  for (auto& R0Coefficient : g_R0MercuryCoefficients)
    R0 += (R0Coefficient.A*cos(R0Coefficient.B + (R0Coefficient.C*rho)));

  //Calculate R1
  double R1{0};
  for (auto& R1Coefficient : g_R1MercuryCoefficients)
    R1 += (R1Coefficient.A*cos(R1Coefficient.B + (R1Coefficient.C*rho)));

  //Calculate R2
  double R2{0};
  for (auto& R2Coefficient : g_R2MercuryCoefficients)
    R2 += (R2Coefficient.A*cos(R2Coefficient.B + (R2Coefficient.C*rho)));

  //Calculate R3
  double R3{0};
  for (auto& R3Coefficient : g_R3MercuryCoefficients)
