...
History: PJN / 03-10-2009 1. Fixed a copy and paste gremlin in the CAAEarth::EclipticLatitude method where it incorrectly
         ...
         PJN / 18-03-2012 1. All global "g_*" tables are now. Thanks to Roger Dahl for reporting this 
...
std::array<VSOP87Coefficient, 64> g_L0EarthCoefficients
{ {
  { 175347046, 0,         0            },
...
std::array<VSOP87Coefficient, 34> g_L1EarthCoefficients
{ {
  { 628331966747.0, 0,          0           },
...
std::array<VSOP87Coefficient, 20> g_L2EarthCoefficients
{ {
  { 52919,  0,      0         },
...
std::array<VSOP87Coefficient, 7> g_L3EarthCoefficients
{ {
  { 289, 5.844, 6283.076 },
  { 35,  0,     0        },
  { 17,  5.49,  12566.15 },
...
std::array<VSOP87Coefficient, 3> g_L4EarthCoefficients
{ {
  { 114, 3.142,  0        },
  { 8,   4.13,   6283.08  },
  { 1,   3.84,   12566.15 }
...
std::array<VSOP87Coefficient, 1> g_L5EarthCoefficients
{ {
  { 1, 3.14, 0 }
} };

std::array<VSOP87Coefficient, 5> g_B0EarthCoefficients
{ {
  { 280, 3.199, 84334.662 },
  { 102, 5.422, 5507.553  },
  { 80,  3.88,  5223.69   },
...
std::array<VSOP87Coefficient, 2> g_B1EarthCoefficients
{ {
  { 9, 3.90, 5507.55 },
  { 6, 1.73, 5223.69 }
} };

std::array<VSOP87Coefficient, 40> g_R0EarthCoefficients
{ {
  { 100013989,  0,          0            },
...
std::array<VSOP87Coefficient, 10> g_R1EarthCoefficients
{ {
...
std::array<VSOP87Coefficient, 6> g_R2EarthCoefficients
{ {
...
std::array<VSOP87Coefficient, 2> g_R3EarthCoefficients
{ {
  { 145,  4.273,  6283.076 },
  { 7,    3.92,   12566.15 }
} };

std::array<VSOP87Coefficient, 1> g_R4EarthCoefficients
{ {
  { 4, 2.56, 6283.08 }
} };

std::array<VSOP87Coefficient, 34> g_L1EarthCoefficientsJ2000
{ {
  { 628307584999.0, 0,          0           },
  { 206059,         2.678235,   6283.075850 },
  { 4303,           2.6351,     12566.1517  },
...
std::array<VSOP87Coefficient, 20> g_L2EarthCoefficientsJ2000
{ {
...
std::array<VSOP87Coefficient, 7> g_L3EarthCoefficientsJ2000
{ {
  { 289,  5.842,  6283.076 },
  { 21,   6.05,   12566.15 },
  { 3,    5.20,   155.42   },
...
std::array<VSOP87Coefficient, 2> g_L4EarthCoefficientsJ2000
{ {
  { 8,  4.14, 6283.08  },
  { 1,  3.28, 12566.15 }
} };

std::array<VSOP87Coefficient, 7> g_B1EarthCoefficientsJ2000
{ {
...
std::array<VSOP87Coefficient, 4> g_B2EarthCoefficientsJ2000
{ {
...
std::array<VSOP87Coefficient, 3> g_B3EarthCoefficientsJ2000
{ {
  { 276,  0.595,  6283.076 },
  { 17,   3.14,   0        },
  { 4,    0.12,   12566.15 }
...
std::array<VSOP87Coefficient, 2> g_B4EarthCoefficientsJ2000
{ {
  { 6,  2.27, 6283.08 },
  { 1,  0,    0       }
} };
...
double CAAEarth::EclipticLatitude(double JD, bool bHighPrecision)
{
  ...
  //Calculate B0
  double B0{0};
  for (auto& B0Coefficient : g_B0EarthCoefficients)
    B0 += (B0Coefficient.A*cos(B0Coefficient.B + (B0Coefficient.C*rho)));

  //Calculate B1
  double B1{0};
  for (auto& B1Coefficient : g_B1EarthCoefficients)
...
double CAAEarth::EclipticLongitudeJ2000(double JD, bool bHighPrecision)
{
  ...
  //Calculate L1
  double L1{0};
  for (auto& L1Coefficient : g_L1EarthCoefficientsJ2000)
    L1 += (L1Coefficient.A*cos(L1Coefficient.B + (L1Coefficient.C*rho)));

  //Calculate L2
  double L2{0};
  for (auto& L2Coefficient : g_L2EarthCoefficientsJ2000)
    L2 += (L2Coefficient.A*cos(L2Coefficient.B + (L2Coefficient.C*rho)));

  //Calculate L3
  double L3{0};
  for (auto& L3Coefficient : g_L3EarthCoefficientsJ2000)
    L3 += (L3Coefficient.A*cos(L3Coefficient.B + (L3Coefficient.C*rho)));

  //Calculate L4
  double L4{0};
  for (auto& L4Coefficient : g_L4EarthCoefficientsJ2000)
    L4 += (L4Coefficient.A*cos(L4Coefficient.B + (L4Coefficient.C*rho)));
...
double CAAEarth::EclipticLatitudeJ2000(double JD, bool bHighPrecision)
{
  ...
  //Calculate B1
  double B1{0};
  for (auto& B1Coefficient : g_B1EarthCoefficientsJ2000)
    B1 += (B1Coefficient.A*cos(B1Coefficient.B + (B1Coefficient.C*rho)));

  //Calculate B2
  double B2{0};
  for (auto& B2Coefficient : g_B2EarthCoefficientsJ2000)
    B2 += (B2Coefficient.A*cos(B2Coefficient.B + (B2Coefficient.C*rho)));

  //Calculate B3
  double B3{0};
  for (auto& B3Coefficient : g_B3EarthCoefficientsJ2000)
    B3 += (B3Coefficient.A*cos(B3Coefficient.B + (B3Coefficient.C*rho)));

  //Calculate B4
  double B4{0};
  for (auto& B4Coefficient : g_B4EarthCoefficientsJ2000)
