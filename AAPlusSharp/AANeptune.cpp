...
std::array<VSOP87Coefficient, 38> g_L0NeptuneCoefficients
{ {
  { 531188633,  0,          0          },
...
std::array<VSOP87Coefficient, 18> g_L1NeptuneCoefficients
{ {
  { 3837687717.0, 0,        0        },
...
std::array<VSOP87Coefficient, 7> g_L2NeptuneCoefficients
{ {
  { 53893, 0,     0      },
  { 296,   1.855, 1.484  },
  { 281,   1.191, 38.133 },
...
std::array<VSOP87Coefficient, 4> g_L3NeptuneCoefficients
{ {
  { 31, 0,    0     },
  { 15, 1.35, 76.27 },
  { 12, 6.04, 1.48  },
...
std::array<VSOP87Coefficient, 1> g_L4NeptuneCoefficients
{ {
  { 114, 3.142,  0 }
} };

std::array<VSOP87Coefficient, 17> g_B0NeptuneCoefficients
{ {
...
std::array<VSOP87Coefficient, 13> g_B1NeptuneCoefficients
{ {
...
std::array<VSOP87Coefficient, 6> g_B2NeptuneCoefficients
{ {
  { 9691, 5.5712, 38.1330 },
  { 79,   3.63,   76.27   },
  { 72,   0.45,   36.65   },
...
std::array<VSOP87Coefficient, 4> g_B3NeptuneCoefficients
{ {
  { 273, 1.017, 38.133 },
  { 2,   0,     0      },
  { 2,   2.37,  36.65  },
...
std::array<VSOP87Coefficient, 1> g_B4NeptuneCoefficients
{ {
  { 6, 2.67, 38.13 }
} };

std::array<VSOP87Coefficient, 32> g_R0NeptuneCoefficients
{ {
  { 3007013206.0, 0,          0           },
...
std::array<VSOP87Coefficient, 15> g_R1NeptuneCoefficients
{ {
...
std::array<VSOP87Coefficient, 5> g_R2NeptuneCoefficients
{ {
  { 4247, 5.8991, 38.1330 },
  { 218,  0.346,  1.484   },
  { 163,  2.239,  168.053 },
...
std::array<VSOP87Coefficient, 1> g_R3NeptuneCoefficients
{ {
  { 166,  4.552,  38.133 }
} };
...
double CAANeptune::RadiusVector(double JD, bool bHighPrecision)
{
  ...
  //Calculate R0
  double R0{0};
  for (auto& R0Coefficient : g_R0NeptuneCoefficients)
    R0 += (R0Coefficient.A*cos(R0Coefficient.B + (R0Coefficient.C*rho)));

  //Calculate R1
  double R1{0};
  for (auto& R1Coefficient : g_R1NeptuneCoefficients)
    R1 += (R1Coefficient.A*cos(R1Coefficient.B + (R1Coefficient.C*rho)));

  //Calculate R2
  double R2{0};
  for (auto& R2Coefficient : g_R2NeptuneCoefficients)
    R2 += (R2Coefficient.A*cos(R2Coefficient.B + (R2Coefficient.C*rho)));

  //Calculate R3
  double R3{0};
  for (auto& R3Coefficient : g_R3NeptuneCoefficients)
