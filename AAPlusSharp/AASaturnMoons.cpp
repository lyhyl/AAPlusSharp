...
History: PJN / 09-02-2004 1. Updated the values used in the calculation of the a1 and a2ants
                          for Rhea (satellite V) following an email from Jean Meeus confirming
                          that theseants are indeed incorrect as published in the book.
...
void CAASaturnMoons::HelperSubroutine(double e, double lambdadash, double p, double a, double omega, double i, double c1, double s1, double% r, double% lambda, double% gamma, double% w)
{
  ...
  double cosgrad{cos(grad)};
  double singrad{sin(grad)};
  double irad{CAACoordinateTransformation::DegreesToRadians(i)};
  double sinirad{sin(irad)};
  double cosirad{cos(irad)};
  double a1{sinirad*singrad};
  double a2{(c1*sinirad*cosgrad) - (s1*cosirad)};
  ...
  double h{(c1*sinirad) - (s1*cosirad*cosgrad)};
  double psirad{atan2(s1*singrad, h)};
...
CAASaturnMoonsDetails CAASaturnMoons::CalculateHelper(double JD, double sunlongrad, double betarad, double R, bool bHighPrecision)
{
  ...
  double a1{(0.000265*sin(pdashrad)) + (0.001*sin(W4rad))}; //Note the book uses the incorrectant 0.01*sin(W4rad);
  double a2{(0.000265*cos(pdashrad)) + (0.001*cos(W4rad))}; //Note the book uses the incorrectant 0.01*cos(W4rad);
  ...
  double sinidashrad{sin(idashrad)};
  double cosidashrad{cos(idashrad)};
  ...
  double b1{sinidashrad*sin(omegadashrad - W8rad)};
  double b2{(cos(W7rad)*sinidashrad*cos(omegadashrad - W8rad)) - (sin(W7rad)*cosidashrad)};
  ...
  double cosgamma1rad{cos(gamma1rad)};
  double X1{r1*((cosu*cosw) - (sinu*cosgamma1rad*sinw))};
  double Y1{r1*((sinu*cosw*cosgamma1rad) + (cosu*sinw))};
  double Z1{r1*sinu*sin(gamma1rad)};
  ...
  double cosgamma2rad{cos(gamma2rad)};
  double X2{r2*((cosu*cosw) - (sinu*cosgamma2rad*sinw))};
  double Y2{r2*((sinu*cosw*cosgamma2rad) + (cosu*sinw))};
  double Z2{r2*sinu*sin(gamma2rad)};
  ...
  double cosgamma3rad{cos(gamma3rad)};
  double X3{r3*((cosu*cosw) - (sinu*cosgamma3rad*sinw))};
  double Y3{r3*((sinu*cosw*cosgamma3rad) + (cosu*sinw))};
  double Z3{r3*sinu*sin(gamma3rad)};
  ...
  double cosgamma4rad{cos(gamma4rad)};
  double X4{r4*((cosu*cosw) - (sinu*cosgamma4rad*sinw))};
  double Y4{r4*((sinu*cosw*cosgamma4rad) + (cosu*sinw))};
  double Z4{r4*sinu*sin(gamma4rad)};
  ...
  double cosgamma5rad{cos(gamma5rad)};
  double X5{r5*((cosu*cosw) - (sinu*cosgamma5rad*sinw))};
  double Y5{r5*((sinu*cosw*cosgamma5rad) + (cosu*sinw))};
  double Z5{r5*sinu*sin(gamma5rad)};
  ...
  double cosgamma6rad{cos(gamma6rad)};
  double X6{r6*((cosu*cosw) - (sinu*cosgamma6rad*sinw))};
  double Y6{r6*((sinu*cosw*cosgamma6rad) + (cosu*sinw))};
  double Z6{r6*sinu*sin(gamma6rad)};
  ...
  double cosgamma7rad{cos(gamma7rad)};
  double X7{r7*((cosu*cosw) - (sinu*cosgamma7rad*sinw))};
  double Y7{r7*((sinu*cosw*cosgamma7rad) + (cosu*sinw))};
  double Z7{r7*sinu*sin(gamma7rad)};
  ...
  double cosgamma8rad{cos(gamma8rad)};
  double X8{r8*((cosu*cosw) - (sinu*cosgamma8rad*sinw))};
  double Y8{r8*((sinu*cosw*cosgamma8rad) + (cosu*sinw))};
  double Z8{r8*sinu*sin(gamma8rad)};
...
void CAASaturnMoons::Rotations(double X, double Y, double Z, double c1, double s1, double c2, double s2, double lambda0, double beta0, double% A4, double% B4, double% C4)
{
  ...
  double A3{(A2*sinlambda0) - (B2*coslambda0)};
  double B3{(A2*coslambda0) + (B2*sinlambda0)};
  double C3{C2};

  double cosbeta0{cos(beta0)};
  double sinbeta0{sin(beta0)};
  A4 = A3;
