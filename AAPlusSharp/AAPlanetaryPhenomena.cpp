...
std::array<PlanetaryPhenomenaCoefficient1, 14> g_PlanetaryPhenomenaCoefficient1
{ {
...
double CAAPlanetaryPhenomena::K(double Year, Planet planet, Type type)
{
...
double CAAPlanetaryPhenomena::Mean(double k, Planet planet, Type type)
{
  size_t nCoefficient{0};
  if (planet >= Planet::MARS)
  {
...
double CAAPlanetaryPhenomena::True(double k, Planet planet, Type type)
{
  double JDE0{0};
...
double CAAPlanetaryPhenomena::ElongationValue(double k, Planet planet, bool bEastern)
{
  double JDE0{Mean(k, planet, Type::INFERIOR_CONJUNCTION)};

  assert(planet < Planet::MARS);

  size_t nCoefficient{static_cast<size_t>(planet)*2};
  assert(nCoefficient < g_PlanetaryPhenomenaCoefficient1.size());
#ifdef _MSC_VER
  #pragma warning(suppress : 26446 26482)
#endif //#ifdef _MSC_VER
