...
CAA2DCoordinate CAAPrecession::AdjustPositionUsingMotionInSpace(double r, double DeltaR, double t, double Alpha, double Delta, double PMAlpha, double PMDelta)
{
  ...
  double rCosDelta{r*cosDelta};

  double x{rCosDelta*cosAlpha};
  double y{rCosDelta*sinAlpha};
  double z{r*sin(Delta)};

  double DeltaX{((x/r)*DeltaR) - (z*PMDelta*cosAlpha) - (y*PMAlpha)};
  double DeltaY{((y/r)*DeltaR) - (z*PMDelta*sinAlpha) + (x*PMAlpha)};
  double DeltaZ{((z/r)*DeltaR) + (r*PMDelta*cosDelta)};
...
CAA2DCoordinate CAAPrecession::PrecessEquatorial(double Alpha, double Delta, double JD0, double JD)
{
  ...
  double cosAlphaplussigma{cos(Alpha + sigma)};
  double A{cosDelta*sin(Alpha + sigma)};
  double B{(cosphi*cosDelta*cosAlphaplussigma) - (sinphi*sinDelta)};
  double C{(sinphi*cosDelta*cosAlphaplussigma) + (cosphi * sinDelta)};
...
CAA2DCoordinate CAAPrecession::PrecessEcliptic(double Lambda, double Beta, double JD0, double JD)
{
  ...
  double sinpiminusLambda{sin(pi - Lambda)};
  ...
  double A{(coseta*cosBeta*sinpiminusLambda) - (sineta*sinBeta)};
  double B{cosBeta*cos(pi - Lambda)};
  double C{(coseta*sinBeta) + (sineta*cosBeta*sinpiminusLambda)};
