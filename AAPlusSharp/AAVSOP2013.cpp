...
bool CAAVSOP2013EphemeridesFile::ReadTextFile(std::filesystem::path::value_type* pszFilename)
{
  ...
  if (!std::getline(file, sLine))
    return false;
  int nIdentificationIndex{std::atoi(sLine.c_str())};
  ...
  if (!std::getline(file, sLine))
    return false;
  double fSizeBasicInterval{std::stod(sLine)};
  if (fSizeBasicInterval != SIZE_BASIC_INTERVAL)
    return false;
  if (!std::getline(file, sLine))
    return false;
  int nChebyshevTables{std::atoi(sLine.c_str())};
  if (nChebyshevTables != CHEBYSHEV_TABLES)
    return false;
  if (!std::getline(file, sLine))
    return false;
  int nCoefficientsPerTable{std::atoi(sLine.c_str())};
...
size_t fwrite_int32_t(int32_t nValue, FILE* f)
{
  uint32_t nTemp{htonl(nValue)};
  return fwrite(&nTemp, sizeof(nTemp), 1, f);
}


//A function to convert a big endian uint32_t to a host endian int32_t
inline int32_t convert_read_int32_t(uint32_t% nValue)
{
  uint32_t nTemp{ntohl(nValue)};
  if (nTemp > INT32_MAX)
#ifdef _MSC_VER
...
size_t fread_int32_t(int32_t% nValue, FILE* f)
{
  uint32_t nTemp{0};
  auto nRead{fread(&nTemp, sizeof(nTemp), 1, f)};
  if (nRead != 1)
    return nRead;
...
bool CAAVSOP2013EphemeridesFile::WriteBinaryFile(std::filesystem::path::value_type* pszFilename)
{
  FILE* f{nullptr};
#ifdef _MSC_VER
...
#pragma warning(suppress : 6387)
  ...
  if (fwrite(&m_fEndJD, sizeof(m_fEndJD), 1, f) != 1)
    return false;
  for (auto& coefficient : m_FirstCoefficientRank)
  {
    if (fwrite_int32_t(coefficient, f) != 1)
      return false;
  }
  for (auto& coeff : m_CoefficientsPerCoordinate)
  {
    if (fwrite_int32_t(coeff, f) != 1)
      return false;
  }
  for (auto& subInterval : m_SubIntervals)
  {
...
bool CAAVSOP2013EphemeridesFile::ReadBinaryFile(std::filesystem::path::value_type* pszFilename)
{
  FILE* f{nullptr};
#ifdef _MSC_VER
...
bool CAAVSOP2013ElementsFile::ReadTextFile(std::filesystem::path::value_type* pszFilename)
{
  std::ifstream file{pszFilename};
  if (!file)
    return false;
...
#pragma warning(disable : 26446)
    ...
    for (int i{0}; i<nt; i++)
    {
      ...
      if (nRank != (i + 1))
        return false;
      series.m_Terms->Add(term);
    }

    m_AllSeries->Add(series);
...
bool CAAVSOP2013ElementsFile::WriteBinaryFile(std::filesystem::path::value_type* pszFilename)
{
  FILE* f{nullptr};
#ifdef _MSC_VER
  if (_wfopen_s(&f, pszFilename, L"wb") != 0)
...
#pragma warning(suppress : 26472)
#endif //#ifdef _MSC_VER
  auto nSeries{static_cast<int32_t>(m_AllSeries.size())};
  if (fwrite_int32_t(nSeries, f) != 1)
    return false;
  for (auto& series : m_AllSeries)
  {
...
#pragma warning(suppress : 26472)
#endif //#ifdef _MSC_VER
    auto nTerms{static_cast<int32_t>(series.m_Terms.size())};
    if (fwrite_int32_t(nTerms, f) != 1)
      return false;
    for (auto& term : series.m_Terms)
    {
      for (auto phi : term.m_iphi)
      {
...
bool CAAVSOP2013ElementsFile::ReadBinaryFile(std::filesystem::path::value_type* pszFilename)
{
  FILE* f{nullptr};
#ifdef _MSC_VER
  if (_wfopen_s(&f, pszFilename, L"rb") != 0)
...
void CAAVSOP2013::SetBinaryFilesDirectory(std::filesystem::path::value_type* pszBinaryFilesDirectory)
{
  m_pszBinaryFilesDirectory = pszBinaryFilesDirectory;
}

std::filesystem::path::value_type* CAAVSOP2013::GetBinaryFilesDirectory()
{
...
CAAVSOP2013Position CAAVSOP2013::Calculate(Planet planet, double JD)
{
  ...
  auto iter{std::upper_bound(m_DateRange.cbegin(), m_DateRange.cend(), JD)};
  assert(iter != m_DateRange.cend());
  size_t nIndex{iter - m_DateRange.cbegin() - size_t{1}};
...
#pragma warning(suppress : 26446 26482)
  ...
  int iper{static_cast<int>((JD - file.m_fStartJD)/CAAVSOP2013EphemeridesFile::SIZE_BASIC_INTERVAL)}; //Calculate 32 day index
  assert((iper >= 0) && (iper < static_cast<int>(file.m_ChebyshevTables.size())));
#ifdef _MSC_VER
#pragma warning(disable : 26446 26482)
#endif //#ifdef _MSC_VER
  auto& aperiod{file.m_ChebyshevTables[iper]}; //Get table containing JD
  auto iad{file.m_FirstCoefficientRank[static_cast<int>(planet)] - 1}; //Initial term of the planet (index into m_Coefficients)
  auto ncf{file.m_CoefficientsPerCoordinate[static_cast<int>(planet)]}; //Number of Chebyshev coefficients
  auto nsi{file.m_SubIntervals[static_cast<int>(planet)]}; //Some delta divider
  auto delta2{static_cast<double>(CAAVSOP2013EphemeridesFile::SIZE_BASIC_INTERVAL/nsi)};
...
#pragma warning(suppress : 26472)
#endif //#ifdef _MSC_VER
  size_t iloc{iad + (6*static_cast<size_t>(ncf)*ik)};
  double dj0{aperiod[0] + (ik*delta2)};
  double x{(2.0*(JD - dj0)/delta2) - 1.0};
...
#pragma warning(suppress : 26472)
    ...
    {
      size_t jp{ncf - j - 1}; //index into tn, working backwards
      assert((jp >= 0) && (jp < tn.size()));
#ifdef _MSC_VER
#pragma warning(suppress : 26472)
#endif //#ifdef _MSC_VER
      size_t jt{iloc + (static_cast<size_t>(ncf)*i) + jp + 2}; //Index into coefficients
...
#pragma warning(disable : 26446 26482)
  ...
  std::array<double, 6> r{};
  for (auto& serie : file.m_AllSeries)
  {
    double fSum{0};
    for (auto& term : serie.m_Terms)
    {
      double fPhi{0};
      for (int i{0}; i<17; i++)
      {
...
void CAAVSOP2013::CalculateLambdas(double T, std::array<double, 17>% lambdas)
{
#ifdef _MSC_VER
#pragma warning(disable : 26446)
#endif //#ifdef _MSC_VER
  ...
  lambdas[13] = 0.3595362285049309*T; //� Pluto
...
CAAEllipticalObjectElements CAAVSOP2013::OrbitToElements(double JD, CAAVSOP2013::Planet planet, CAAVSOP2013Orbit% orbit)
{
  ...
  double w{atan2(orbit.h, orbit.k)}; //here w is longitude of perihelion
  ...
  double MeanMotion{CalculateMeanMotion(planet, orbit.a)};
...
CAAVSOP2013Position CAAVSOP2013::Ecliptic2Equatorial(CAAVSOP2013Position% value)
{
  double coeff11{0.99999999999996836};
  double coeff12{2.3076633339445195e-07};
  double coeff13{-1.0004940139786859e-07};
  double coeff21{-2.5152133775962465e-07};
  double coeff22{0.91748213272857493};
  double coeff23{-0.39777699295429642};
  double coeff32{0.39777699295430902};
  double coeff33{0.91748213272860391};
